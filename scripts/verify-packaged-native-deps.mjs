...
export function verifyPackagedNativeDeps({ appOutDir, platform, arch }) {
  ...
  requireFile(index, errors, 'LICENSE');
  requireFile(index, errors, 'FONT_LICENSES.md');
  const appPackage = readJson(index, errors, 'package.json');
  validatePackageMetadata(errors, appPackage, 'pi-desktop', 'package.json');
  ...
  if (nodePtyBase) {
    if (targetPlatform === 'win32') {
      ...
      requireTargetBinary(index, errors, `${nodePtyBase}/spawn-helper`, targetPlatform, targetArch);
    }
  }
