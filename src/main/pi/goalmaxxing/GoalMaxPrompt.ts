...
export function goalMaxCapsule(goal: GoalMaxState, recovery?: GoalMaxRecovery): string {
  ...
  const capsule = [
    ...
    'User messages use the normal session transport: steering arrives during work, follow-ups arrive after the current turn. Address delivered messages before completing the goal; never wait for goal completion to process them.',
    ...
    '4. Respect user messages delivered through the session: process steering now and follow-ups when delivered. Reconcile changed scope into the task plan before completing the goal.',
    '',
    'NEXT-TURN CONTRACT',
    instruction,
    needsTaskPlan
