...
export async function buildSubagentCatalog(
  ...
  const capabilities = [
    ...
    'Resource and turn thresholds: maxCostUsd, maxInputTokens, maxOutputTokens, maxTotalTokens, and maxTurns are advisory liveness checkpoints. Crossing any threshold reports to the parent and never stops a child or workflow.',
    ...
    'Notifications: never; next-turn adds model-visible parent context without starting a turn; immediate triggers a parent turn or queues a follow-up if the parent is streaming.',
    ...
    'Liveness: timeoutSeconds and idleTimeoutSeconds are opt-in advisory notification thresholds, never automatic stops. Progress, sustained low-diversity repetition, liberal checkpoints, and adaptive turn thresholds produce structured parent reports while children continue by default.',
