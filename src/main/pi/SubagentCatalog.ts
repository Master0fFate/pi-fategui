...
export async function buildSubagentCatalog(
  ...
  const capabilities = [
    'Agent Teams V2: spawn_agent accepts optional workspace {mode: shared|worktree, baseRef?, branch?}. Omission uses the live global workspace preference; a soft policy permits explicit per-spawn override while a strict policy rejects incompatible modes. get_agent_workspace_policy reports the current policy; Settings > Agent is the only way to change it. Shared inherits the caller checkout; worktree creates an isolated local checkout from committed history. agent_workspace reviews, checkpoints, explicitly integrates (ff-only or selected cherry-picks with reviewed HEADs), or safely removes a closed child checkout while retaining its branch. Worktrees are not security sandboxes; full-access remains unsandboxed. Depth is limited to 2, non-root nodes to 16, and active child turns to 3 per team; writer leases are per checkout.',
    'The routing, budgets, mailboxes, workflows, and uncapped batch behavior below describe legacy subagents, not Agent Teams V2. Legacy subagents do not support per-child worktrees.',
