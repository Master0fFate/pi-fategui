...
export function registerIpc({ runtime, projects, files, git, settings, terminal, logs, music, speech, rendererPolicy }: IpcServices): void {
  ...
  handle(ipcChannels.runtimeNewSession, async (_event, input) => {
    emptyInputSchema.parse(input);
    return runRuntimeMutation('creating a session', async () => {
      const defaults = await settings.load();
      return runtimeStateSchema.parse(await runtime.newSession({ thinkingLevel: defaults.thinkingLevel, defaultModel: defaults.defaultModel }));
    });
  });
