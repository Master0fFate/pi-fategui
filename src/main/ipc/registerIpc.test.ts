...
import { activatePreparedProject, assertProjectActivationIdle, createProjectActivationQueue, discardCreatedWorktreeAfterFailure } from './registerIpc';
...
function state(project: ProjectState | null, status = 'ready', overrides: Record<string, unknown> = {}) {
  return { project, status, streaming: false, sessionOperation: false, error: null, ...overrides } as never;
}
