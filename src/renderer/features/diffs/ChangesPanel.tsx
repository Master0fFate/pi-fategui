  ...
  return (
    <div className="commit-card" aria-label={`Commit details for ${commit.subject}`}>
      ...
      <div className="commit-card-actions">
        ...
        {details?.githubUrl ? <button type="button" onClick={() => window.open(details.githubUrl!, '_blank', 'noopener,noreferrer')}><GithubMark size={13} /><span className="icon-label">Open on GitHub</span></button> : <span className="commit-card-no-remote">No GitHub remote</span>}
      </div>
    </div>
  );
}

/** The GitHub brand mark was removed from lucide in v1; lucide's migration
 *  guide says to inline the official brand SVG instead. */
