...
describe('declarative skin contracts', () => {
  it('accepts known presets and bounded layout options', () => {
    ...
    expect(builtInSkins.find((skin) => skin.id === 'm3-expressive')?.name).toBe('M3 Expressive');
    expect(skinIdSchema.parse('m3-expressive')).toBe('m3-expressive');
    expect(builtInSkins[0]?.id).toBe('default');
    expect(skinIdSchema.parse('pack:test-pack')).toBe('pack:test-pack');
    expect(skinIdSchema.parse('legacy-missing')).toBe('default');
  });
  it.each(['../outside', '/absolute', 'C:\\outside', 'CON', 'con', 'aux', 'com1', 'dreamcore', 'angelcore', 'm3-expressive', 'a.b'])('rejects unsafe or reserved folder ID %s', (id) => {
  ...
  it.each([{ css: '*{display:none}' }, { script: 'run.js' }, { schemaVersion: 3 }, { base: 'custom-react' }, { base: 'm3-expressive' }, { layout: { contentWidth: 1 } }, { layout: { opacity: 0 } }, { background: { file: '../secret.png' } }, { background: { file: 'https://example.com/a.png' } }])('rejects unsupported or executable fields %j', (extra) => {
