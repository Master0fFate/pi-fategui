...
- Package/OS version and `PRODVER` use the numeric release version, currently **1.1.0 in the source tree**. The optional `package.json.releaseName` adds the display suffix **Axiom** for 1.1.0, so its title is **V1.1.0 - Axiom**. A source version is not proof that an installer has been published.
- A release tag must be `v<package.json version>` (for example `v1.1.0`) and point to history contained in `main`. The human release title is derived separately by `node scripts/release-artifacts.mjs metadata --field display-version`.
...
Cross-platform or native dependency changes must pass the GitHub Actions matrix. Do not claim another operating system was verified solely from a cross-compiled artifact. Before tagging 1.1.0, verify the source is committed to `main`, the [1.1.0 release notes](../.github/release-notes/v1.1.0.md) match the shipped behavior, the Windows/macOS/Linux native package and installed-smoke jobs pass, and staged installers have all seven expected assets plus `SHA256SUMS`. A local test pass is not a release sign-off; do not create the tag or publish artifacts until these gates pass.
